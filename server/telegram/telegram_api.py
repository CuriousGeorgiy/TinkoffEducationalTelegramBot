...
class TelegramAPI:
    ...
    def __init__(self, people_sheet, push_notifications_sheet, faq_sheet, classes_schedule_sheet, config,
        ...
        self._create_mappings_from_people_sheet(people_sheet)
    ...
    def _create_mappings_from_people_sheet(self, people_sheet):
        self._phone_number_to_id_dict = {}
        self._id_to_person_info_dict = {}

        for i in range(1, len(people_sheet)):
            self._phone_number_to_id_dict[people_sheet[i][0]] = i
            self._id_to_person_info_dict[i] = {'name': people_sheet[i][1], 'groups': set()}
            for j in range(2, len(people_sheet[0])):
                if int(people_sheet[i][j]):
                    self._id_to_person_info_dict[i]['groups'].add(people_sheet[0][j])

        for telegram_id, user_data in self._updater.dispatcher.user_data.items():
            try:
                if user_data['authorized']:
                    self._id_to_person_info_dict[user_data['id_in_telegram_api']]['telegram_id'] = telegram_id
            except KeyError:
                pass
        ...
        for text, groups, send_time in self._push_notifications_sheet:
            ...
            if send_time.timestamp() > datetime.datetime.today().timestamp():
                self._push_notification_jobs.append(self._updater.job_queue.run_once(
                    ...
                    context=(self._id_to_person_info_dict, groups)))
    ...
    def _remove_mappings_for_people_sheet(self):
        self._phone_number_to_id_dict.clear()
        self._id_to_person_info_dict.clear()
    ...
    def _update_mappings_for_people_sheet(self, people_sheet):
        self._remove_mappings_for_people_sheet()
        self._create_mappings_from_people_sheet(people_sheet)
    ...
    def update_data_sheets(self, people_sheet, push_notifications_sheet, faq_sheet, classes_schedule_sheet):
        self._update_mappings_for_people_sheet(people_sheet)
    ...
    def set_person_telegram_id(self, person_id, telegram_id):
        self._id_to_person_info_dict[person_id]['telegram_id'] = telegram_id

    """ Get methods """

    def get_mappings_for_people_sheet(self):
        return self._phone_number_to_id_dict, self._id_to_person_info_dict
